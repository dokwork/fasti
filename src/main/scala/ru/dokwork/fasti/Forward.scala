...
private[fasti] object Forward {
  ...
  implicit final class ForwardOps[F[_], A, B, S <: HList](val self: Forward[F, A, B, S]) {
    ...
    def continue[P <: HList](
        p: P
    )(implicit F: MonadError[F, Throwable], ev: BeginFrom[S, P]): F[Either[(HList, Throwable), B]] = {
      require(ev ne null)
    ...
    def andThen[C, S2 <: HList](
        other: Forward[F, B, C, S2]
    )(implicit ev: Prepend[S, B :: S2]): Forward[F, A, C, ev.Out] = {
      require(ev ne null)
  ...
  private case class Action[F[_], +A, B](
      run: Any => F[B],
      isFatal: PartialFunction[Throwable, Unit] = PartialFunction.empty
  ) extends Forward[F, A, B, HNil]

  private case class Next[F[_], A, B, C, S <: HList](first: Action[F, A, B], next: UntypedForward[F, C])
      extends Forward[F, A, C, B :: S]

  def apply[F[_], A, B](f: A => F[B]): Forward[F, A, B, HNil] = Action[F, A, B](f.asInstanceOf[Any => F[B]])
  ...
  private def execute[F[_], B](completed: HList)(f0: UntypedForward[F, B], p0: HList)(
    ...
    (f0, p0) match {
      case (Action(f, classifier), x :: HNil) => attempt(f(x), classifier)
      case (Next(Action(f, classifier), f2), x :: HNil) =>
  ...
  private def andThen[F[_]](f1: UntypedForward[F, _], f2: UntypedForward[F, _]): UntypedForward[F, _] = f1 match {
    case r @ Action(_, _) => Next(r, f2)
    case Next(r, n)       => Next(r, andThen(n, f2))
  }

  private def breakOn[F[_], B](
      f0: UntypedForward[F, B]
  )(isFatal: PartialFunction[Throwable, Unit]): UntypedForward[F, B] =
    f0 match {
      case r @ Action(_, _) => r.copy(isFatal = isFatal)
