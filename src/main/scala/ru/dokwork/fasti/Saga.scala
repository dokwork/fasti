...
final class Saga[F[_], A, B, C <: Coproduct] private(
  private val run: Forward[F, A, B, C],
  private val compensate: Backward[F, B, C]
  ...
  def continue[Encoded](completedStages: NonEmptyList[Encoded])(
    implicit
    decoder: DecodeStages[Encoded, C]
  ): F[Result[B]] = {
  ...
  private def continue0(done: List[C], result: F[run.Result]): F[Result[B]] =
    F.flatMap(result) {
      case (_, Right(b)) =>
  ...
  private def rollback0(stages: List[C], cause: Throwable): F[Result.Rolledback[B]] = {
    import scala.collection.immutable.::

    def loop(stages: List[C]): F[Result.Rolledback[B]] = stages match {
      case head :: tail =>
  ...
  def andThen[C1 <: Coproduct, D](other: Saga[F, B, D, C1])(
    implicit
    join: Join[C, B :+: C1]
  ): Saga[F, A, D, join.Out] =
